...
def load_save():
    """Load high score and seed from save.txt.
    Falls back to legacy highscore.txt if save.txt doesn't exist yet."""
    path = _get_save_path()
    score = 0
    seed  = ""
    try:
        with open(path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                if line.startswith("score="):
                    score = max(0, int(line[6:]))
                elif line.startswith("seed="):
                    seed = line[5:].strip()
    except FileNotFoundError:
        # migrate from old highscore.txt
        legacy = _get_highscore_path()
        try:
            with open(legacy, "r") as f:
                score = max(0, int(f.read().strip()))
            print(f"Migrated legacy highscore.txt -> save.txt . RIP highscore.txt (score={score})")
        except Exception:
            pass
...
def save_save(score, seed):
    ...
    try:
        ...
        with open(path, "w", encoding="utf-8") as f:
            ...
            f.write(f"seed={seed}\n")
        seed_tag = f" | seed='{seed}'" if seed else " | no seed"
        print(f"Save made: score={score}{seed_tag}")
    except Exception as e:
...
def _seed_int():
    """Convert ACTIVE_SEED string to a stable integer for random.seed()."""
    if not ACTIVE_SEED:
        return None
...
pygame.display.set_caption("NotASnake v3.2 | Thanks for playing!")
...
# ── Load seed from save at startup ───────────────────────────────
_boot_score, _boot_seed = load_save()
ACTIVE_SEED = _boot_seed
if ACTIVE_SEED:
    print(f"=== SEED LOADED FROM SAVE: '{ACTIVE_SEED}' (int={_seed_int()}) ===")
else:
...
def reset_single_game():
    ...
    if ACTIVE_SEED:
        print(f"=== GAME START | seed='{ACTIVE_SEED}' int={_seed_int()} | food counter reset ===")
    else:
        print("=== GAME START | no seed — random bullshit go! ===")
...
def reset_multiplayer_game():
    ...
    _food_call_counter = 0
    if ACTIVE_SEED:
        print(f"=== MULTI GAME START | seed='{ACTIVE_SEED}' int={_seed_int()} | food counter reset ===")
    else:
        print("=== MULTI GAME START | no seed — random bullshit go! ===")
...
def show_score(choice, color, font, size, score1=None, score2=None, game_start_ms=None):
    ...
    if choice == 1:
        ...
        if score2 is None:
            score_text = f"NotASnake v3.2 | End Score: {score1}"
            score_surface = score_font.render(score_text, True, txt_color)
            screen.blit(score_surface, (SCREEN_WIDTH//2 - 250, SCREEN_HEIGHT * 4 // 5))
        else:
            score_text = f"NotASnake v3.2 | How cool! Two guys playing! | P1: {score1} | P2: {score2}"
            score_surface = score_font.render(score_text, True, txt_color)
...
def pause_menu(is_multiplayer=False):
    ...
    while True:
        ...
        info_text = f"Mode: {mode_text}. Uhh, it`s 5 o`clock somewhere, am i right? "
...
def game_over_menu(is_multiplayer=False, score1=0, score2=0, high_score=0, winner=None):
    ...
    while True:
        ...
        if is_multiplayer:
            ...
            score_color = COLORS["ourple"] if score1 >= high_score and score1 > 0 else txt
...
def seed_input_overlay():
    """Blocking text-input overlay for entering/clearing the active seed.
    Returns the new seed string (empty = cleared)."""
    global ACTIVE_SEED
    font_big  = pygame.font.SysFont("consolas", 36)
    font_med  = pygame.font.SysFont("consolas", 22)
    font_sm   = pygame.font.SysFont("consolas", 17)
    MAX_LEN   = 20
    buf       = list(ACTIVE_SEED)   # start with whatever is already set
    ...
    while True:
        ...
        hint = font_sm.render("No symbols|  max 20 chars  |  ENTER=confirm  ESC=cancel", True, (140,140,160))
...
def main_menu():
    ...
    while True:
        ...
        # ── Title & high score ──
        title_surface = pygame.font.SysFont("times new roman", 100).render("NotASnake v3.2", True, COLORS["green"])
        ...
        if panel_x < SCREEN_WIDTH:
            ...
            def draw_setting(y, label, val_str, val_col, desc_lines):
                ...
                screen.blit(vs, vr)
                # Only render first desc line to keep rows compact
                if desc_lines:
                    ds = controls_font.render(desc_lines[0], True, (140, 140, 160))
                    screen.blit(ds, (px, y + 26))
                row_h = 50   # fixed compact height per row
                div_y = y + row_h
