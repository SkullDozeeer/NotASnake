...
def load_save():
    path = _get_save_path()
    data = {"score": 0, "seed": "", "double_food": True,
        ...
        with open(path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                if line.startswith("score="):
                    data["score"] = max(0, int(line[6:]))
                elif line.startswith("seed="):
                    data["seed"] = line[5:].strip()
                elif line.startswith("double_food="):
                    data["double_food"] = line[12:].strip() == "1"
                elif line.startswith("apples="):
                    val = line[7:].strip()
                    if val:
                        data["apples"] = set(int(x) for x in val.split(",") if x.strip().isdigit())
                elif line.startswith("sa_best_"):
                    parts = line.split("=", 1)
                    if len(parts) == 2:
                        try:
                            t = int(parts[0][8:])
                            data[f"sa_best_{t}"] = int(parts[1]) if parts[1].strip() else 0
                        except ValueError:
                            pass
...
def save_save(data):
    ...
    try:
        ...
        with open(path, "w", encoding="utf-8") as f:
            ...
            f.write(f"double_food={'1' if data.get('double_food', True) else '0'}\n")
...
pygame.display.set_caption("NotASnake v3.3 | Thanks for playing!")
...
settings = {
    ...
    "light_mode":       False,
    "grid_opacity":     0,
    "control_scheme":   "wasd_arrows",
    "show_timer":       False,
    "fps_limit":        60,
    "hardcore":         False,
    "double_food":      True,
}
settings["double_food"] = _boot_data.get("double_food", True)
...
def show_score(choice, color, font, size, score1=None, score2=None, game_start_ms=None):
    ...
    if choice == 1:
        ...
        if score2 is None:
            score_text = f"NotASnake v3.3 | End Score: {score1}"
            score_surface = score_font.render(score_text, True, txt_color)
            screen.blit(score_surface, (SCREEN_WIDTH//2 - 250, SCREEN_HEIGHT * 4 // 5))
        else:
            score_text = f"NotASnake v3.3 | How cool! Two guys playing! | P1: {score1} | P2: {score2}"
            score_surface = score_font.render(score_text, True, txt_color)
...
def pause_menu(is_multiplayer=False, mode_label=None):
    ...
    while True:
        ...
        info_text = f"Mode: {mode_label}. Uhh, it`s 5 o`clock somewhere, am i right? "
...
def sa_success_screen(target, elapsed_ms, is_new_best, is_new_apple):
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_RETURN: return "play_again"
...
def sp_mode_select():
    ...
    while True:
        ...
        # ── Endless panel ────────────────────────────────────────
        left_cx = mx // 2
        endless_hover = mouse_pos[0] < mx - 6 and mouse_pos[1] > 105
        if endless_hover:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_F5:
                    ...
                    if not settings["hardcore"]: return "menu"
...
def main_menu():
    ...
    while True:
        ...
        # ── Title & high score ──
        title_surface = pygame.font.SysFont("times new roman", 100).render("NotASnake v3.3", True, COLORS["green"])
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if gear_rect.collidepoint(event.pos):
                    ...
                    if setting_rects.get("music") and setting_rects["music"].collidepoint(event.pos):
                        ...
                        _d = load_save(); _d["double_food"] = settings["double_food"]; save_save(_d)
