...
def _log_mode_launch(mode_name):
    print(f"=== MODE LAUNCHED: {mode_name} | {datetime.datetime.now()} ===")

def _log_focus_event(kind, source):
    print(f"[FOCUS] {kind} via {source} @ {datetime.datetime.now()}")

def _log_all_events_for_focus_debug(event):
    """Logs every raw pygame event type once seen, so focus-loss signaling
    gaps on unusual hardware (e.g. SDL not emitting ACTIVEEVENT/WINDOWFOCUSLOST)
    show up in the debug log instead of silently doing nothing."""
    et = event.type
    if et == pygame.ACTIVEEVENT:
        _log_focus_event("ACTIVEEVENT gain=%s state=%s" % (event.gain, event.state), "ACTIVEEVENT")
    elif et == pygame.WINDOWFOCUSLOST:
        _log_focus_event("WINDOWFOCUSLOST", "WINDOWFOCUSLOST")
    elif et == pygame.WINDOWFOCUSGAINED:
        _log_focus_event("WINDOWFOCUSGAINED", "WINDOWFOCUSGAINED")
    elif et == pygame.WINDOWMINIMIZED:
        _log_focus_event("WINDOWMINIMIZED", "WINDOWMINIMIZED")
# ────────────────────────────────────────────────────────────
...
VERSION       = "3.5.2"
BUILD_EDITION = ""
VERSION_STR   = f"{GAME_NAME} v{VERSION}" + (f" {BUILD_EDITION}" if BUILD_EDITION else "")
print(f"=== Running {VERSION_STR} ===")
...
_ACH_FLAG_KEYS = [
    "ach_pressed_space_no_wildcard", "ach_died_instant", "ach_hardcore_start",
    "ach_fog_start", "ach_seeded_game", "ach_rewind_full_charge", "ach_rewind_no_charge",
    "ach_mp_solo_death", "ach_mp_tie_zero_food", "ach_nondefault_setting_play",
    "ach_manual_screenshot", "ach_secret_50",
] + [f"ach_mode_start_{m}" for m in ["endless", "sa", "shrink", "deathmatch", "pacifist", "trust", "chaos", "rewind", "mp"]]

_save_cache = None

def _copy_save_data(d):
...
def load_save(path=None):
    ...
    if use_cache and _save_cache is not None:
        return _copy_save_data(_save_cache)
    if path is None:
        path = _get_save_path()
    data = {
        ...
        "bg_style": "plain", "seen_tips": set(), "achievements": set(),
    }
    for k in _STATS_INT_KEYS:
        data[k] = 0
    for k in _ACH_FLAG_KEYS:
        data[k] = False
    try:
        with open(path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                if key == "score":
                    ...
                    if val:
                        data["achievements"] = set(x for x in val.split(",") if x.strip())
                elif key in _ACH_FLAG_KEYS:
                    data[key] = val == "1"
        ...
        if os.path.exists(bak):
            try:
                ...
                print("save.txt was gone, pulled it back from save.txt.bak")
                _save_cache = None
                return load_save()
            except Exception as e:
                print(f"bak restore borked: {e}")
    ...
    if use_cache:
        _save_cache = _copy_save_data(data)
    return data
...
def save_save(data):
    ...
    try:
        ...
        with open(tmp_path, "w", encoding="utf-8") as f:
            ...
            ach_str = ",".join(sorted(data.get("achievements", set())))
            f.write(f"achievements={ach_str}\n")
            for k in _ACH_FLAG_KEYS:
...
def _load_achievements_module():
    if getattr(sys, 'frozen', False):
        ...
        base_dir = os.path.dirname(os.path.abspath(__file__))
    ach_path = os.path.join(base_dir, "nasassets", "achievements.py")
    import importlib.util
    old_dont_write = sys.dont_write_bytecode
    sys.dont_write_bytecode = True
    try:
        spec = importlib.util.spec_from_file_location("nas_achievements", ach_path)
...
class _AchievementsStub:
    ACHIEVEMENTS = []
    def compute_unlocked(self, data):
        return set()
    def get_achievement(self, aid):
        return None
    def ordered_ids(self, ids):
        return []
    def _has_nondefault_setting(self, data):
        return False

try:
    _ach = _load_achievements_module()
except Exception as _ach_load_err:
    print(f"achievements.py did not load: {_ach_load_err}")
    print("No achievements this session. Game still runs.")
    _ach = _AchievementsStub()

_achievement_toast_queue = []
_ACHIEVEMENT_TOAST_MS = 3200
_ach_space_flag_done = [False]
_ach_screenshot_flag_done = [False]

def _queue_achievement_toasts(ids):
    queued = False
    for aid in _ach.ordered_ids(ids):
        a = _ach.get_achievement(aid)
        if a:
            # Start the lifetime only when this toast reaches the front of the queue.
            _achievement_toast_queue.append({"name": a["name"], "start_ms": None, "until_ms": None})
            queued = True
    if queued:
        music_manager.play_achievement_sound()

def draw_achievement_toasts():
    global _achievement_toast_queue
    now_ms = pygame.time.get_ticks()
    if not _achievement_toast_queue:
        return
    toast = _achievement_toast_queue[0]
    if toast["start_ms"] is None:
        toast["start_ms"] = now_ms
        toast["until_ms"] = now_ms + _ACHIEVEMENT_TOAST_MS
    if now_ms >= toast["until_ms"]:
        _achievement_toast_queue.pop(0)
        if not _achievement_toast_queue:
            return
        toast = _achievement_toast_queue[0]
        toast["start_ms"] = now_ms
    ...
    age = now_ms - toast["start_ms"]
    alpha = 255
    if age < 250:
        alpha = int(255 * (age / 250))
    elif remaining < 400:
        alpha = int(255 * (remaining / 400))
    title_s = get_font("ui_bold", 18, bold=True).render("Achievement Unlocked", True, COLORS["yummers"])
    name_s  = get_font("ui", 20).render(toast["name"], True, COLORS["white"])
    pad = 14
    bw = max(title_s.get_width(), name_s.get_width()) + pad * 2
    bh = title_s.get_height() + name_s.get_height() + pad * 2 + 4
    bx = SCREEN_WIDTH // 2 - bw // 2
    by = 20
    panel = pygame.Surface((bw, bh), pygame.SRCALPHA)
    panel.fill((15, 15, 30, min(220, alpha)))
    panel.blit(title_s, (pad, pad))
    panel.blit(name_s, (pad, pad + title_s.get_height() + 4))
    panel.set_alpha(alpha)
    screen.blit(panel, (bx, by))
    pygame.draw.rect(screen, COLORS["purpleguy"], (bx, by, bw, bh), 2)

_screenshot_flash_until_ms = 0
_screenshot_flash_time = ""

def take_manual_screenshot(score_value=0, tag="manual"):
    save_score_screenshot(score_value, tag)
    flash_screenshot_saved()
    if _ach_screenshot_flag_done[0]:
        return
    _ach_screenshot_flag_done[0] = True
    d = load_save()
    if not d.get("ach_manual_screenshot", False):
        d["ach_manual_screenshot"] = True
        save_save(d)
        check_achievements(d)

def flash_screenshot_saved():
    global _screenshot_flash_until_ms, _screenshot_flash_time
    _screenshot_flash_until_ms = pygame.time.get_ticks() + 2000
    _screenshot_flash_time = datetime.datetime.now().strftime("%H:%M:%S")

def draw_screenshot_flash():
    if pygame.time.get_ticks() >= _screenshot_flash_until_ms:
        return
    ss_font = get_font("ui", 16)
    ss_surf = ss_font.render(f"Screenshot saved  {_screenshot_flash_time}", True, (200, 255, 200))
    screen.blit(ss_surf, ss_surf.get_rect(bottomright=(SCREEN_WIDTH - 12, SCREEN_HEIGHT - 12)))

def check_achievements(data=None):
    if data is None:
        data = load_save()
    unlocked_now = _ach.compute_unlocked(data)
    already = data.get("achievements", set())
    new_ones = unlocked_now - already
    if new_ones:
        data["achievements"] = already | unlocked_now
        save_save(data)
        _queue_achievement_toasts(new_ones)
    return new_ones, data

def record_death(elapsed_ms, fog_on=False):
    data = load_save()
    if elapsed_ms <= 5000:
        data["ach_died_instant"] = True
    if fog_on:
        data["fog_deaths"] = data.get("fog_deaths", 0) + 1
    save_save(data)
    new_ones, _ = check_achievements(data)
    return new_ones

def record_mode_start(mode_key, seeded=False, hardcore=False, fog=False):
    data = load_save()
    data[f"ach_mode_start_{mode_key}"] = True
    if seeded:
        data["ach_seeded_game"] = True
    if hardcore:
        data["ach_hardcore_start"] = True
    if fog:
        data["ach_fog_start"] = True
    if _ach._has_nondefault_setting(data):
        data["ach_nondefault_setting_play"] = True
    save_save(data)
    new_ones, _ = check_achievements(data)
    return new_ones
...
def fuse_save_data(a, b):
    ...
    result["seen_tips"] = set(a.get("seen_tips", set())) | set(b.get("seen_tips", set()))

    # Achievement state is monotonic, so fuse flags and the persisted ID cache
    # instead of letting save A silently discard achievements from save B.
    for k in _ACH_FLAG_KEYS:
        result[k] = bool(a.get(k, False) or b.get(k, False))
    result["achievements"] = (
        set(a.get("achievements", set())) | set(b.get("achievements", set()))
    )
    ...
    # Recompute the canonical achievement ID set from the fused state. This
    # repairs stale/missing IDs while preserving historical progress.
    result["achievements"] |= _ach.compute_unlocked(result)
    return result
...
pygame.display.set_caption(f"{VERSION_STR} | Thanks for playing!")
...
class MusicManager:
    ...
    def play_achievement_sound(self, volume=0.45):
        if not self.music_enabled or settings.get("music_muted", False):
            return
        try:
            sample_rate = 22050
            notes = [(784.0, 0.075), (988.0, 0.075), (1319.0, 0.17)]
            samples = []
            for freq, dur in notes:
                n = max(1, int(sample_rate * dur))
                for i in range(n):
                    t = i / n
                    env = min(1.0, t / 0.06) * ((1.0 - t) ** 1.4)
                    samples.append(int(21000 * env * math.sin(2 * math.pi * freq * i / sample_rate)))
            ...
            print(f"Achievement jingle borked: {e}")
...
def _reload_state_from_save():
    ...
    settings["fog_of_war"]     = data.get("fog_of_war", False)
    settings["start_length"]   = data.get("start_length", 3)
    settings["snake_pattern"]  = data.get("snake_pattern", True)
    settings["bg_style"]       = data.get("bg_style", "plain")
    return data
...
def fuse_save_files(path_a, path_b):
    try:
        ...
        _, fused = check_achievements(fused)
        save_save(fused)
        _reload_state_from_save()
...
def show_score(choice, color, font, size, score1=None, score2=None, game_start_ms=None):
    ...
    if choice == 1:
        ...
        if score2 is None:
            score_text = f"{VERSION_STR} | End Score: {score1}"
            score_surface = score_font.render(score_text, True, txt_color)
            screen.blit(score_surface, (SCREEN_WIDTH//2 - 250, SCREEN_HEIGHT * 4 // 5))
        else:
            score_text = f"{VERSION_STR} | How cool! Two guys playing! | P1: {score1} | P2: {score2}"
            score_surface = score_font.render(score_text, True, txt_color)
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                elif event.key == pygame.K_F12: take_manual_screenshot(0, "paused")
                elif event.key == pygame.K_RETURN: return sel == 1
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                elif event.key == pygame.K_F12: take_manual_screenshot(score1 + score2 if is_multiplayer else score1, "gameover")
            if event.type == pygame.JOYBUTTONDOWN:
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                elif event.key == pygame.K_F12: take_manual_screenshot(target, "scoreattack")
            if event.type == pygame.JOYBUTTONDOWN:
...
def sp_mode_select():
    ...
    MODES = [
        ...
        {"key": "shrink",   "label": "Shrink",        "color": (255, 140, 0),
         "desc": ["Snake shrinks every 5s.", "Eat to survive."]},
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_F12:
                    take_manual_screenshot(0, "modes")
                elif event.key == pygame.K_ESCAPE:
                    return "back"
...
def score_attack_game(target):
    ...
    _log_mode_launch(f"Score Attack ({target})")
    record_mode_start("sa", seeded=bool(ACTIVE_SEED), hardcore=settings["hardcore"], fog=settings["fog_of_war"])
    ...
    while True:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.state == pygame.APPINPUTFOCUS and not event.gain:
                    _log_all_events_for_focus_debug(event)
        ...
        if game_over:
            ...
            record_death(now_ms - game_start_ms, fog_on=settings['fog_of_war'])
            pixel_fill_effect()
...
def shrink_game():
    ...
    global leftover, burst1, debug_overlay_visible, _spacebar_idx
    global _shrink_best
    _log_mode_launch("Shrink")
    record_mode_start("shrink", seeded=bool(ACTIVE_SEED), hardcore=settings["hardcore"], fog=settings["fog_of_war"])
...
def deathmatch_game():
    ...
    global leftover, burst1, debug_overlay_visible, _spacebar_idx
    global _dm_best_ms
    _log_mode_launch("Deathmatch")
    record_mode_start("deathmatch", seeded=bool(ACTIVE_SEED), hardcore=settings["hardcore"], fog=settings["fog_of_war"])
    music_manager.play_heroic(volume=0.5)
    reset_single_game()
    _spacebar_idx = 0
    draw_countdown()
    ...
    while True:
        ...
        if game_over:
            ...
            if is_new_best:
                ...
                save_save(data)
            record_game_stats(elapsed_ms, score)
            record_death(elapsed_ms, fog_on=settings["fog_of_war"])
            pixel_fill_effect()
        ...
        version_str = f"{VERSION_STR}  [DEATHMATCH]"
...
def pacifist_percent_game():
    ...
    _log_mode_launch("Pacifist%")
    record_mode_start("pacifist", seeded=bool(ACTIVE_SEED), hardcore=settings["hardcore"], fog=settings["fog_of_war"])
    music_manager.play_heroic(volume=0.5)
    reset_single_game()
    score = 0
    _spacebar_idx = 0
...
def trust_issues_game():
    ...
    _log_mode_launch("Trust Issues")
    record_mode_start("trust", seeded=bool(ACTIVE_SEED), hardcore=settings["hardcore"], fog=settings["fog_of_war"])
    music_manager.play_heroic(volume=0.5)
    reset_single_game()
    score = 0
    _spacebar_idx = 0
...
def chaos_mode_game():
    ...
    _log_mode_launch("Chaos Mode")
    record_mode_start("chaos", seeded=bool(ACTIVE_SEED), hardcore=settings["hardcore"], fog=settings["fog_of_war"])
    ...
    try:
        ...
        def _reroll():
            ...
            for k in keys:
                ...
                settings[k] = new_val
                if k == "control_scheme" and new_val == "spacebar":
                    nonlocal_spacebar_target = direction
            ...
            if nonlocal_spacebar_target is not None:
                global _spacebar_idx
                _spacebar_idx = _SPACEBAR_CYCLE.index(nonlocal_spacebar_target)
...
def rewind_game():
    ...
    _log_mode_launch("Rewind")
    record_mode_start("rewind", seeded=bool(ACTIVE_SEED), hardcore=settings["hardcore"], fog=settings["fog_of_war"])
    music_manager.play_heroic(volume=0.5)
    reset_single_game()
    score = 0
    _spacebar_idx = 0
    ...
    while True:
        ...
        if game_over:
            ...
            if charges == MAX_CHARGES:
                _d["ach_rewind_full_charge"] = True
            if charges == 0:
                _d["ach_rewind_no_charge"] = True
            save_save(_d)
            check_achievements(_d)
            pixel_fill_effect()
...
def achievements_screen():
    title_font = get_font("title", 56)
    row_title_font = get_font("ui_bold", 20, bold=True)
    row_desc_font  = get_font("ui", 17)
    back_font  = get_font("ui", 26)
    btn_font   = get_font("menu", 28)
    small_font = get_font("ui", 15)

    scroll = 0
    max_scroll = 0
    launch_msg = ""
    launch_msg_until_ms = 0
    tracker_hit = pygame.Rect(0, 0, 0, 0)

    while True:
        data = load_save()
        unlocked = _ach.compute_unlocked(data)
        draw_background()
        lm  = settings["light_mode"]
        txt = (20, 20, 20) if lm else COLORS["white"]
        dim = (80, 80, 80) if lm else (140, 140, 140)
        ...
        title_s = title_font.render("Achievements", True, COLORS["purpleguy"])
        ...
        count_s = row_desc_font.render(f"{len(unlocked)} / {len(_ach.ACHIEVEMENTS)} unlocked", True, COLORS["green"])
        count_r = count_s.get_rect(topright=(SCREEN_WIDTH - 22, 20))
        screen.blit(count_s, count_r)

        tracker_hover = tracker_hit.collidepoint(mouse_pos)
        tracker_s = btn_font.render("[ Live Tracker ]", True, COLORS["cyan"] if tracker_hover else txt)
        tracker_r = tracker_s.get_rect(topright=(SCREEN_WIDTH - 22, 60))
        screen.blit(tracker_s, tracker_r)
        tracker_hit = pygame.Rect(tracker_r.left - 10, tracker_r.top - 6, tracker_r.width + 20, tracker_r.height + 12)

        content_top = 110
        content_bottom = SCREEN_HEIGHT - 24
        ...
        for a in _ach.ACHIEVEMENTS:
            ...
            if is_unlocked:
                dot_col = COLORS["green"]
                name_col = COLORS["yummers"]
                desc_txt = a.get("desc_unlocked", a["desc"])
                desc_col = txt
            elif is_secret:
                dot_col = dim
                name_col = dim
            ...
            pygame.draw.circle(screen, dot_col, (col_x + 8, y + 12), 6)

            name_s = render_fit("ui_bold", a["name"] if (is_unlocked or not is_secret) else "???",
                                 name_col, col_w - 30, 20, min_size=13, bold=True)
            screen.blit(name_s, (col_x + 26, y))

            desc_s = render_fit("ui", desc_txt, desc_col, col_w - 30, 17, min_size=12)
            screen.blit(desc_s, (col_x + 26, y + 24))

            pygame.draw.line(screen, (60, 60, 80), (col_x, y + row_h - 8), (col_x + col_w, y + row_h - 8), 1)
            y += row_h
        ...
        if max_scroll > 0:
            hint_s = small_font.render("Scroll wheel / arrows to see more", True, dim)
            screen.blit(hint_s, hint_s.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT - 10)))

        if launch_msg and pygame.time.get_ticks() < launch_msg_until_ms:
            lm_s = small_font.render(launch_msg, True, COLORS["yummers"])
            screen.blit(lm_s, lm_s.get_rect(center=(SCREEN_WIDTH // 2, content_top - 20)))
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if back_hit.collidepoint(event.pos):
                    return
                elif tracker_hit.collidepoint(event.pos):
                    ok = launch_achievement_tracker()
                    launch_msg = "Tracker window opened." if ok else "Couldn't open tracker — check the log."
                    launch_msg_until_ms = pygame.time.get_ticks() + 2500
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_ESCAPE:
                    return
                elif event.key == pygame.K_F12:
                    take_manual_screenshot(0, "screen")
                elif event.key == pygame.K_DOWN:
...
def launch_achievement_tracker():
    try:
        if getattr(sys, 'frozen', False):
            base_dir = os.path.dirname(sys.executable)
        else:
        ...
        if getattr(sys, 'frozen', False):
            for candidate in (
                os.path.join(base_dir, "achievements_tracker.exe"),
                os.path.join(base_dir, "nasassets", "achievements_tracker.exe"),
            ):
                if os.path.exists(candidate):
                    subprocess.Popen([candidate], cwd=base_dir)
                    return True
            print("Frozen build and no achievements_tracker.exe next to the game")
            return False
        tracker_path = os.path.join(base_dir, "nasassets", "achievements_tracker.py")
        if not os.path.exists(tracker_path):
            print(f"No tracker script at {tracker_path}")
            return False
        subprocess.Popen([sys.executable, tracker_path], cwd=base_dir)
        return True
    except Exception as e:
        print(f"Tracker launch failed: {e}")
        return False
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN and event.key == pygame.K_F12:
                take_manual_screenshot(0, "stats")
...
def how_to_play_screen():
    ...
    SECTIONS = [
        ("Controls", [
            ...
            "Main menu: A opens Achievements, T opens Stats.",
            "F12 grabs a screenshot anywhere: in a run, paused, or in any menu.",
        ]),
        ("Modes", [
...
def main_menu():
    ...
    while True:
        ...
        title_surface = render_fit("title", VERSION_STR, COLORS["green"], title_max_w, 100, min_size=40)
        ...
        ach_surf = gear_font.render("[ Achievements ]", True, txt)
        ach_rect = ach_surf.get_rect(topleft=(16, 16 + stats_rect.height + 6))
        if ach_rect.collidepoint(mouse_pos):
            ach_surf = gear_font.render("[ Achievements ]", True, COLORS["purpleguy"])
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if stats_rect.collidepoint(event.pos):
                    stats_screen()
                elif ach_rect.collidepoint(event.pos):
                    achievements_screen()
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_LEFT:
                    ...
                    take_manual_screenshot(load_high_score(), "menu")
            ...
            if event.type == pygame.JOYBUTTONDOWN:
                if _joy_is_debug_btn(event):
                    ...
                    toggle_panel()
                elif _joy_is_pause_btn(event) and not panel_open:
                    achievements_screen()
                elif panel_open:
                    if event.button in (0, 2):
...
def single_player_game():
    _log_mode_launch("Endless")
    record_mode_start("endless", seeded=bool(ACTIVE_SEED), hardcore=settings["hardcore"], fog=settings["fog_of_war"])
    music_manager.play_music('ingame', loop=True, volume=0.5)
    global snake_pos, snake_body, food_pos, food_spawn, food2_pos, direction, change_to, score
    global leftover, burst1
    ...
    while True:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if settings["control_scheme"] == "spacebar":
                    ...
                    if (event.key == pygame.K_SPACE
                            and settings["control_scheme"] != "spacebar"
                            and not _ach_space_flag_done[0]):
                        _ach_space_flag_done[0] = True
                        _d = load_save()
                        if not _d.get("ach_pressed_space_no_wildcard", False):
                            _d["ach_pressed_space_no_wildcard"] = True
                            save_save(_d)
                            check_achievements(_d)
                if event.key == pygame.K_F5:
                    debug_overlay_visible = not debug_overlay_visible
        ...
        if do_step:
            ...
            if not moment_shown and score >= MOMENT_SCORE:
                ...
                _d = load_save(); _d["ach_secret_50"] = True; save_save(_d)
                check_achievements(_d)
...
def multiplayer_game():
    _log_mode_launch("Local Multiplayer")
    record_mode_start("mp", seeded=bool(ACTIVE_SEED), hardcore=settings["hardcore"], fog=settings["fog_of_war"])
    music_manager.play_music('ingame', loop=True, volume=0.5)
    global snake1_pos, snake1_body, snake2_pos, snake2_body, food_pos, food2_pos, food_spawn
    global direction1, change_to1, direction2, change_to2, score1, score2
    global leftover, burst1, burst2
    ...
    while True:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_F5:
                    ...
                    take_manual_screenshot(score1 + score2)
                elif event.key == pygame.K_ESCAPE:
                    if pause_menu(is_multiplayer=True, mode_label="Multiplayer"):
                        return "menu"
                elif event.key == pygame.K_m:
        ...
        if p1_game_over or p2_game_over:
            ...
            _d = load_save()
            if p1_game_over != p2_game_over:
                _d["ach_mp_solo_death"] = True
            if winner == "tie" and score1 == 0 and score2 == 0:
                _d["ach_mp_tie_zero_food"] = True
            save_save(_d)
            check_achievements(_d)
            pixel_fill_effect()
